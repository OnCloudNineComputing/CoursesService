...
class CoursesTAResource(BaseRDBApplicationResource):

    def __init__(self):
        super().__init__()

    @classmethod
    def get_by_id(cls, course_id, order_by=None, limit=None,
                         offset=None, field_list=None):
        db_name, table_name = CoursesTAResource.get_data_resource_info()
        res = RDBService.find_by_template(db_name, table_name,
                                          {"course_id": course_id}, order_by, limit,
                                                  offset, field_list)
        return res

    @classmethod
    def delete_by_id(cls, template):
        db_name, table_name = CoursesTAResource.get_data_resource_info()
