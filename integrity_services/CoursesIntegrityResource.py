...
class CoursesIntegrity(BaseIntegrityResource):
    ...
    def type_validation(cls, data):
        ...
        try:
            if "professor" in input_fields:
                prof = data["professor"]
                if type(prof) != str:
                    raise ValueError("Invalid entry for professor, must be a string.")
        except ValueError as v:
        ...
        try:
            if "course_days" in input_fields:
                ...
                if type(days) != str or re.match("^[MTWRFO]+$", days) is None:
                    raise ValueError("Invalid entry for course days; acceptable values are any combination of " +
        ...
        try:
            if "start_time" in input_fields:
                ...
                try:
                    ...
                    raise ValueError("Invalid start time; must be a string in this format: " +
        ...
        try:
            if "end_time" in input_fields:
                ...
                try:
                    ...
                    raise ValueError("Invalid end time; must be a string in this format: " +
                                     "'Hour{1-12}:Minute{00-59} AM/PM'.")
        except ValueError as v:
        ...
        try:
            if "location" in input_fields:
                loc = data["location"]
                if type(loc) != str:
                    raise ValueError("Invalid entry for location, must be a string.")
        except ValueError as v:
        ...
        try:
            for r in CoursesIntegrity.required_fields:
                if r not in input_fields:
                    raise ValueError("Missing required data fields; course_name, course_year, course_sem, dept, " +
                                     "course_number, section, professor, credits, and course_days are required.")
        except ValueError as v:
    ...
    def course_collection_put_responses(cls, res):
        ...
        if status == 422:
            ...
            rsp = Response("Failed! Matching courses not found or unexpected error.",
                           status=422, content_type="text/plain")
    ...
    def course_by_id_get_responses(cls, res):
        ...
        if status == 200:
            ...
            rsp = Response("Failed! Course ID not found.", status=status, content_type="text/plain")
    ...
    def course_by_id_put_responses(cls, res, course_id):
        ...
        if status == 422:
            ...
            rsp = Response("Success! The given data for the course with ID " +
                           str(course_id) + " was updated as requested.", status=status,
                           content_type="text/plain")
        else:
    ...
    def course_by_id_delete_responses(cls, res, course_id):
        status = CoursesIntegrity.delete_responses(res)
        if status == 204:
            rsp = Response("Success! Deleted course with ID " + str(course_id) + ".",
                           status=status, content_type="text/plain")
        else:
    ...
    def course_by_code_get_responses(cls, res):
        ...
        if status == 200:
            ...
            rsp = Response("Failed! Course code not found.", status=status, content_type="text/plain")
    ...
    def course_by_code_put_responses(cls, res, course_code):
        ...
        if status == 422:
            ...
            rsp = Response("Success! The given data for the course with code " +
                           str(course_code) + " was updated as requested.", status=status,
                           content_type="text/plain")
        else:
    ...
    def course_by_code_delete_responses(cls, res, course_code):
        status = CoursesIntegrity.delete_responses(res)
        if status == 204:
            rsp = Response("Success! Deleted course with code " + str(course_code) + ".",
                           status=status, content_type="text/plain")
        else:
